#!/bin/bash
# ============================================================
# NEIA COMFYUI - RunPod Start Script (corrigido)
...
# ----- Instalação de custom node com retry e tratamento de erro -----
install_custom_node() {
    local repo_url="$1"
    local dir_name="$2"
    ...
    if [ -d "$node_dir" ]; then
        ...
        cd "$node_dir" && git pull --quiet 2>/dev/null || echo "[!] Falha ao atualizar $dir_name (ignorando)"
        return 0
    fi
    ...
    for attempt in $(seq 1 $max_retries); do
        if git clone --quiet "$repo_url" "$dir_name" 2>/dev/null; then
            ...
            echo "[!] Tentativa $attempt/$max_retries falhou. Tentando novamente em 5s..."
    ...
    if [ $success -eq 0 ]; then
        echo "[!] Não foi possível clonar $dir_name. Continuando sem este custom node."
        return 0
    fi

    if [ -f "$node_dir/requirements.txt" ]; then
        echo "    [pip] Instalando dependências de $dir_name..."
        pip install -q -r "$node_dir/requirements.txt" 2>/dev/null || echo "    [pip] Falha em algumas dependências de $dir_name"
    fi
...
if [ ! -d "$COMFYUI_DIR" ]; then
    ...
    cd "$COMFYUI_DIR"
    git pull --quiet 2>/dev/null || echo "[!] Falha ao atualizar ComfyUI (ignorando)"
    pip install -q -r requirements.txt --quiet 2>/dev/null || echo "[!] Falha ao instalar requisitos do ComfyUI (ignorando)"
fi
...
# Lista de custom nodes a instalar (repositório, nome da pasta)
...
# Criar symlinks com os nomes esperados pelo workflow
...
wget -q -O "$WORKFLOW_FILE" "$WORKFLOW_URL" && echo "[✓] Workflow salvo em: $WORKFLOW_FILE" || echo "[!] Falha ao baixar workflow, continuando..."
